...
import type { Item3Response } from "./schema.response";
 ...
 * - No gate / no decision
 * - No numeric outputs
 * - No mutation of request inputs
 */

type Tier = "LOW" | "MEDIUM" | "HIGH";

function tierFromAbs(x: number): Tier {
  const a = Math.abs(x);
...
export function runItem3(req: Item3Request): Item3Response {
  ...
  const sources: Array<{
    type: "MISSING" | "CONFLICT" | "AMBIGUOUS" | "ASSUMPTION" | "STALE";
    where: "axis" | "constraint" | "evidence" | "scenario";
    ref?: string;
    signal: string;
  }> = [];
  ...
  let overallBump: Tier | null = null;
  ...
  const hasConstraints =
    !!constraints && ((constraints.hard?.length ?? 0) > 0 || (constraints.soft?.length ?? 0) > 0);
  if (!hasConstraints) {
  ...
  const hardByRef = new Map<string, Set<string>>();
  for (const h of hard) {
    if (!h.ref) continue;
    const set = hardByRef.get(h.ref) ?? new Set<string>();
    set.add(h.name);
  ...
  for (const sc of scenarioSet) {
    ...
    if (aLen >= 3) assumptionHeavyCount++;
    if (!sc.label || sc.label.trim().length < 3) underDefinedCount++;
  }
  ...
  const baseOverallTier = [byAxis.u, byAxis.r, byAxis.i, byAxis.v].reduce(
    (acc, t) => maxTier(acc, t),
    "LOW" as Tier
  );
