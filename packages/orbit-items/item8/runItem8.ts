...
function hasSignal(req: Item8Request, code: string) {
  return (req.signals ?? []).some(s => s.code === code);
}

function anyCodePrefix(req: Item8Request, prefix: string) {
  return (req.signals ?? []).some(s => s.code.startsWith(prefix));
}

export function runItem8(raw: unknown): Item8Result {
  const req = Item8RequestSchema.parse(raw);

  const signals: Item8Result["signals"] = [];

  const stakes = req.meta?.stakes ?? "low";

  const hasResolved =
    hasSignal(req, "resolved") || hasSignal(req, "no_action_needed");
  const hasConflict =
    hasSignal(req, "conflict_detected") ||
    anyCodePrefix(req, "conflict_") ||
    hasSignal(req, "disagree") ||
    hasSignal(req, "models_disagree");
  const hasMissing =
    hasSignal(req, "missing_evidence") ||
    anyCodePrefix(req, "missing_") ||
    hasSignal(req, "constraints_missing");
  const hasHighU =
    hasSignal(req, "u_high") || anyCodePrefix(req, "u_");
