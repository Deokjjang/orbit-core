...
import type { Item5Response } from "./schema.response";

type Tier = "LOW" | "MEDIUM" | "HIGH";
type Verdict = "USE" | "HOLD" | "REJECT";
...
function severityForBool(isBad: boolean, sev: Tier): Tier | null {
...
function hasSelfContradiction(text: string): "OK" | "WARN" | "FAIL" {
  ...
  if (hasBut && hasNeg && hasAff) return "WARN";
  return "OK";
}

function coverageFromQuestions(candidateText: string, questions?: Array<{ text: string }>): "GOOD" | "PARTIAL" | "POOR" {
  if (!questions || questions.length === 0) return "GOOD";
  // heuristic: if candidate is very short relative to question count -> poor
  const len = candidateText.trim().length;
  if (len < 120 && questions.length >= 2) return "POOR";
  if (len < 200 && questions.length >= 3) return "PARTIAL";
  return "PARTIAL";
}

export function runItem5(req: Item5Request): Item5Response {
  const { requestId, candidate, evidenceLinks, uncertainty } = req;

  const reasons: Item5Response["reasons"] = [];
  const needs: Item5Response["needs"] = [];

  // Signals
  const hasCitations = (evidenceLinks?.length ?? 0) > 0 ? "YES" : "NO";
  const consistency = hasSelfContradiction(candidate.text);
  const uTier: Tier = uncertainty?.overallTier ?? "LOW";

  const coverage = coverageFromQuestions(candidate.text, uncertainty?.questions as any);
...
const hasConflictSignal =
  (uncertainty?.signals ?? []).some(
    (s) => typeof s.name === "string" && s.name.includes("conflict")
  ) ?? false;

const conflict: "NONE" | "POSSIBLE" =
  hasConflictSignal ? "POSSIBLE" : "NONE";
  ...
  let verdict: Verdict = "USE";

  // REJECT conditions (strict)
  if (consistency === "FAIL") verdict = "REJECT";
  if (reasons.some((r) => r.signal === "citation_mismatch" && r.severity === "HIGH")) verdict = "REJECT";

  // HOLD conditions
  if (verdict !== "REJECT") {
    if (hasCitations === "NO") verdict = "HOLD";
    if (uTier === "HIGH") verdict = "HOLD";
    if (coverage !== "GOOD") verdict = "HOLD";
    if (reasons.some((r) => r.signal === "overclaim" && (r.severity === "MEDIUM" || r.severity === "HIGH")))
      verdict = "HOLD";
    if (consistency === "WARN") verdict = "HOLD";
  }

  // De-dup needs (by type+text)
  const uniqNeeds: Item5Response["needs"] = [];
