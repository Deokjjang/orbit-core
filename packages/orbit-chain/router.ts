import type { ChainStepId } from "./contract";
import type { PayloadRouter, RouterContext, StepPayload } from "./router.contract";


function pickItem3Thin(outputs: Record<string, unknown>) {
  const o: any = (outputs as any)["item3"];
 ...
 */
export const routerV01: PayloadRouter = {
...
case "item4": {
  ...
  if (looksLikeItemBase) return input;
  ...
  const subjectText =
    input.subject?.kind === "text" ? input.subject.text ?? "" : "";
  ...
  return {
    ...
    uncertainty: pickItem3Thin(outputs as any),
    context: {
      ...
      case "item6_pre":
case "item6_post": {
  const thin = pickItem3Thin(outputs as any);

  // candidates는 기존 로직 유지 (없으면 최소 2개 생성)
  const candidates =
    (input as any).candidates && Array.isArray((input as any).candidates)
      ? (input as any).candidates
      : [
  ...
  const item5Out: any = (outputs as any)["item5"];
    ...
    context: {
      ...
      case "item8":
        return {
          ...
          meta: {
            stakes: (input as any)?.meta?.stakes ?? "low",
          },
        };
  ...
  if (!ctx.input.requestId) return { ok: false, reasons: ["missing_requestId"] };
  ...
  if (allowSkip && !(ctx.outputs as any)?.item5) {
