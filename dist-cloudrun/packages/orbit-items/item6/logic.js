...
function overlapFromJaccard(avg) {
    if (avg >= 0.55)
        return "HIGH";
    if (avg >= 0.30)
        return "MEDIUM";
...
function consensusFromOverlap(ov) {
    if (ov === "HIGH")
        return "AGREE";
    if (ov === "MEDIUM")
        return "PARTIAL";
...
export function runItem6(req) {
    ...
    const overlap = overlapFromJaccard(avgJac);
    const consensus = consensusFromOverlap(overlap);
    ...
    const verdicts = candidates
        ...
        .filter((v) => v === "USE" || v === "HOLD" || v === "REJECT");
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        if (sim >= 0.40)
            groupA.push(candidates[i].id);
        else
            groupB.push(candidates[i].id);
    }
    const clusters = [];
    const mkCluster = (id, ids, summary, sev) => {
        if (ids.length === 0)
            return;
        clusters.push({
            id,
