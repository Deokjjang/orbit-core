import { Item10RequestSchema, Item10ResultSchema } from "./schema";
/* ============================================================
...
function deriveLimits(req) {
    ...
    if (plan === "enterprise")
        return { loopMax: 5, slotsMax: 10, creditsMax: 10000 };
    if (plan === "business")
        return { loopMax: 4, slotsMax: 6, creditsMax: 1000 };
    if (plan === "team")
        return { loopMax: 3, slotsMax: 3, creditsMax: 300 };
    if (plan === "pro")
        return { loopMax: 3, slotsMax: 1, creditsMax: 100 };
    if (plan === "plus")
        return { loopMax: 2, slotsMax: 1, creditsMax: 30 };
    if (plan === "free")
        return { loopMax: 1, slotsMax: 1, creditsMax: 8 };
    return { ...DEFAULT_LIMITS };
}
export function runItem10(raw) {
    ...
    // - ALLOW otherwise
    const verdict = signals.some(s => s.code === "budget_exceeded")
        ? "BLOCK"
        : signals.length > 0
            ? "HOLD"
            : "ALLOW";
