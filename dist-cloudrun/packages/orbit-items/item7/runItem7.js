import { Item7RequestSchema, Item7ResultSchema, } from "./schema";
/* ============================================================
...
function deltas(vals) {
    ...
    for (let i = 1; i < vals.length; i++)
        ds.push(vals[i] - vals[i - 1]);
    return ds;
}
function hasSpike(ds) {
    return ds.some(d => Math.abs(d) >= SPIKE_THRESHOLD);
}
function hasDrift(ds) {
    if (ds.length < DRIFT_MIN_STEPS)
        return false;
