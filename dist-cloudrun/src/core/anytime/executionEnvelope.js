 ...
 */
export const EXECUTION_ENVELOPE_VERSION_V02 = "orbit.exec.v0.2";
// -----------------------------
// Enums / Small Schemas
// -----------------------------
...
export const DeepBundleV02 = z
    ...
    if (d.remainingPlan.length !== expectedRemaining.length ||
    ...
    const shouldBeStopped = d.handledPlanIndex < n && d.handledReasonsByIndex[d.handledPlanIndex] === "UNHANDLED";
 ...
 */
export function createEnvelopeV02(args) {
    const exposure = args.exposure ?? { exposure: "minimal" };
    const deepIn = args.deep ?? {};
    const plan = deepIn.plan ?? [];
    const n = plan.length;
    const handledPlanIndex = deepIn.handledPlanIndex ?? 0;
    const handledReasonsByIndex = deepIn.handledReasonsByIndex ?? new Array(n).fill("UNHANDLED");
    const remainingPlan = deepIn.remainingPlan ?? plan.slice(handledPlanIndex);
    const unitsEstimatedByIndex = deepIn.units?.unitsEstimatedByIndex ?? new Array(n).fill(0);
    const unitsUsedByIndex = deepIn.units?.unitsUsedByIndex ?? new Array(n).fill(0);
