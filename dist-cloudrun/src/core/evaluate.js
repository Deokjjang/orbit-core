...
export function evaluateInternal(requestId, init, constraints, opts) {
    const events = [];
    ...
    // Attractors
    const rawAttractors = cl.clusters.map((c) => {
    ...
    const median = stableSort(rankingInternal, (x) => x.percentile)[Math.floor(rankingInternal.length / 2)]?.percentile ?? 50;
...
export function evaluate(requestId, init, constraints, opts) {
