...
import type { Item1Response, MembersHint } from './types';
import { evaluate } from '../../core/evaluate';

function membersTier(m: number): MembersHint {
  if (m >= 40) return 'LARGE';
  if (m >= 15) return 'MEDIUM';
  return 'SMALL';
}

export function runItem1(req: ItemRequestBase): Item1Response {
  ...
  return {
    ...
    traceKinds: res.trace.events.map((e: any) => e.kind),
  };
}
