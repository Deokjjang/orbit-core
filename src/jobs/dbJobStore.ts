...
import { JobStore, OrbitJob, LeaseOpts } from "./jobStoreTypes";
import { getFirestore } from "../../apps/shared/firebaseAdmin";
import { FieldValue, Timestamp } from "firebase-admin/firestore";

export class DbJobStore implements JobStore {
  private db = getFirestore();
  private jobs = this.db.collection("jobs");
  private idem = this.db.collection("job_idem"); // idempotencyKey -> jobId

  // ---------- helpers ----------
  private toFs(job: OrbitJob) {
    return {
      ...job,
  ...
  await this.db.runTransaction(async tx => {
    ...
    // idempotency ?¥Î? Ï°¥Ïû¨ ???¨ÏÇ¨??
    if (idemSnap && idemSnap.exists) return;
...
async markSucceeded(jobId: string, okEnvelope: any, now: Date): Promise<void> {
  ...
  await this.db.runTransaction(async tx => {
    ...
    const d = snap.data()!;
    if (["SUCCEEDED", "FAILED", "CANCELED"].includes(d.status)) return;
