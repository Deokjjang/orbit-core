...
function resolveGateApprove(): AnyFn {
  ...
  const entryNames = ["approve", "approveGate", "approveExecution"] as const;

  for (const p of candidates) {
    try {
      const mod = req(p) as Record<string, unknown>;
      for (const n of entryNames) {
        if (typeof mod?.[n] === "function") return mod[n] as AnyFn;
      }
    } catch {}
  }
...
export async function approveGateExecution(args: {
  ...
  const q = await db
    ...
    .limit(1)
    .get();
  if (q.empty) throw new Error("evidence_not_found");
