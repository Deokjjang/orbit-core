...
import type { FlGateCaller, FlGateResult } from "./runGate";
import type { FlGateInputEnvelope } from "./gateEnvelope";

type AnyFn = (...args: any[]) => any;

function isFn(x: unknown): x is AnyFn {
 ...
 */
function resolveKernelEntry(): AnyFn {
  ...
  const entryNames = ["runFlGate", "runGate", "evaluate"] as const;
  ...
  for (const modPath of candidates) {
    try {
      ...
      const mod = req(modPath) as Record<string, unknown> | undefined;
      if (!mod) continue;
      ...
      errors.push(
        `Loaded "${modPath}" but no entry found. Exports: ${Object.keys(mod).join(", ")}`
      );
    } catch (e: any) {
